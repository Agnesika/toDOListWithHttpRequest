...
public class TodoService {
    ...
    public void createTodo(Todo todo) throws Exception {
        ...
        String requestsBody = gson.toJson(todo);

        // write http request using HttpRequest class
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("Content-Type", "application/json") // the type of data we are sending in the request additional configurations couldm also be added here
                .POST(HttpRequest.BodyPublishers.ofString(requestsBody)) // the type of request (get, post, put etc) we are making  and the data  to send if data is required
                .build(); // the final copy of the request is compiled based on configuration above and ready  for sending

        // send the request
        HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString()); // the request is sent and the response body converted into String
...
//        System.out.println(response.body());
        ...
        if (createdTodoItem == null || createdTodoItem.get_id() == null) {
            throw new Exception("Failed to create todo item with code: " + response.statusCode()); // throw exception if some issues with the item creation
        }
        // throw an exception if some issues  with creating  the object
